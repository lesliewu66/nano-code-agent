"""
Worktree Manager - Git worktree based task isolation
Each task gets an isolated directory (worktree) for parallel execution.
"""
import json
import os
import re
...
class WorktreeManager:
    ...
    def __init__(self, repo_root: Path, tasks_dir: Path, events: EventBus):
    ...
    def keep(self, name: str) -> str:
        """Mark worktree as kept (don't auto-remove)"""
        wt = self._find(name)
        if not wt:
            return f"Error: Unknown worktree '{name}'"
...
# Initialize (deferred to avoid circular import)
EVENTS = None
WORKTREES = None

def init_worktrees():
    global EVENTS, WORKTREES
    if EVENTS is None:
        from task_system import TASKS_DIR
        EVENTS = EventBus(REPO_ROOT / ".worktrees" / "events.jsonl")
        WORKTREES = WorktreeManager(REPO_ROOT, TASKS_DIR, EVENTS)
